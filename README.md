...
## Utilização
Essa biblioteca é utilizada para, usando a metaprogramação, criar um header personalizado para o seu código, adicionando um serializer e um deserializer de forma automática!

## Exemplo

- **Definição da tabela de campos:** Você precisa criar uma tabela com os campos do seu header, colocando o nome do campo e o seu tipo. Observe, que essa tabela precisa ser declarada como o nome do seu header + `_TABLE_`!
...
- **Classe Builder:** A classe builder faz o processo de deserialização de um header, retornando uma classe object.

- **Funções Default:** A lib apresenta um parser para a maioria dos tipos padrões, caso você deseja adicionar um parser de uma string para o tipo especificado, basta adicionar o seguinte bloco de código, onde dentro dele havrá a lógica específica do tipo escolhido.
